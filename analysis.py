...
#read iris.data file into a dataframe with column names included
...
#data.isnull() Cleaning and detecting missing values , try to find the missing values i.e NaN, which can occur due to several reasons, if there is data is missing, it will display True else False.
#print(df.isnull())
...
#Note if you don't want to use describe function and want to find values for eg min/max from a column, use can use the below, this is an example for sepal length, first create a variable for min/max and include the function
#min_data=df["sepal_length"].min()
#max_data=df["sepal_length"].max()
  
#print("Minimum:",min_data, "\nMaximum:", max_data)

#View the count of each variable in a graph, create a variable for the the dataset to use for graph
